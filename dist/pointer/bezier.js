 ...
 */
export function generateBezier(start, end, sampleCount, rng) {
 ...
 */
export function addOvershoot(mainPath, endpoint, rng) {
    if (mainPath.length < 2)
        return mainPath.slice();
    ...
    if (axisLength < 1e-6)
        return [...mainPath.slice(0, -1), requested];
 ...
 */
export function resamplePath(path, targetCount) {
    if (path.length < 2)
        return path;
    if (targetCount <= 1)
        return [path[0]];
