 ...
 */
export function resample60Hz(path) {
    if (path.length === 0)
        return [];
    ...
    const step = 1000 / 60;
    const targetTimes = [];
    for (let t = first.tMs; t < last.tMs; t += step)
        targetTimes.push(t);
    targetTimes.push(last.tMs);
    const out = [];
    let j = 1;
    for (const targetT of targetTimes) {
        while (j < finitePath.length - 1 && finitePath[j].tMs < targetT)
            j++;
 ...
 */
export function resampleToCount(path, targetCount) {
    if (path.length < 2 || targetCount <= 1)
        return path.slice(0, Math.min(targetCount, path.length));
    const first = path[0];
