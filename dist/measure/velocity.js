 ...
 */
export function classifyVelocityProfile(speeds) {
    ...
    if (peakPos >= 0.2 && peakPos <= 0.8 && rises > 0 && falls > 0 && aboveHalf >= 0.15 && smoothEnough)
        return 'bell';
    if (rises === 0 || falls === 0)
        return 'linear';
    return 'unknown';
...
export function fitSigmaLognormal(speeds) {
    ...
    if (centers.length === 0)
        centers.push(Math.max(0.05, Math.min(0.95, peak)));
    let bestR2 = 0;
    const candidateSets = [centers.slice(0, 1)];
    if (centers.length >= 2)
        candidateSets.push(centers.slice(0, 2));
    if (centers.length >= 3)
        candidateSets.push(centers.slice(0, 3));
    for (const set of candidateSets) {
        for (const sigma of [0.25, 0.4, 0.65, 0.85]) {
            ...
            const coefficients = new Array(basis.length).fill(0);
                ...
                for (let k = 0; k < basis.length; k++) {
                    ...
                    for (let i = 0; i < n; i++) {
                        ...
                        for (let j = 0; j < basis.length; j++)
                            prediction += coefficients[j] * basis[j][i];
                        const b = basis[k][i];
            ...
            if (predMax > 1e-12)
                bestR2 = Math.max(bestR2, computeR2(normalized, predicted.map((value) => value / predMax)));
        }
    }
 ...
 */
export function extractSpeeds(path) {
    if (path.length < 2)
        return [];
    const speeds = [];
