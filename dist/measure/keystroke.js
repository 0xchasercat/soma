 ...
 */
import { classifyDigraph } from '../keystroke/digraph.js';
/**
 ...
 */
function varianceOfClassMeans(groups) {
...
function computeDigraphVariance(keys, flightTimes) {
    if (keys.length < 2 || flightTimes.length < 1)
        return 0;
    const groups = {
        'same-hand': [],
        'cross-hand': [],
...
function computeCapturedDigraphVariance(classes, flightTimes) {
    const groups = {
        'same-hand': [],
        'cross-hand': [],
        'after-space': [],
 ...
 */
export function extractKeystrokeFeatures(keys, holdTimes, flightTimes, correctionCount) {
    const releasePressTimes = flightTimes.map((flightTime, index) => flightTime - (holdTimes[index] ?? 0));
...
export function extractCapturedKeystrokeFeatures(capture) {
    const events = capture.events
        .filter((event) => (event.keyKind === 'character' || event.keyKind === 'space' || event.keyKind === 'punctuation') && Number.isFinite(event.downMs) && Number.isFinite(event.upMs) && event.upMs >= event.downMs)
        .slice()
        .sort((a, b) => a.downMs - b.downMs);
    const holdTimes = events.map((event) => event.upMs - event.downMs);
    const flightTimes = [];
    const releasePressTimes = [];
    const classes = [];
...
//# sourceMappingURL=keystroke.js.map