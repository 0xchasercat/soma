...
function describeKey(char) {
    if (/^[a-z]$/i.test(char))
        return { code: `Key${char.toUpperCase()}`, shiftKey: char !== char.toLowerCase() };
    if (/^[0-9]$/.test(char))
        return { code: `Digit${char}`, shiftKey: false };
    const unshifted = {
    ...
    if (char === 'Backspace')
        return { code: 'Backspace', shiftKey: false };
    if (unshifted[char])
        return { code: unshifted[char], shiftKey: false };
    if (shifted[char])
        return { code: shifted[char], shiftKey: true };
 ...
 */
export function synthesizeKeystrokes(text, profile, seed) {
    ...
    const events = absolute.map((a, index) => {
        const descriptor = describeKey(a.key);
        const correction = a.key === 'Backspace'
            ? { kind: 'backspace', pauseMs: a.pauseAfterMs ?? 0 }
            : index > 0 && absolute[index - 1].key === 'Backspace'
                ? { kind: 'retry', pauseMs: a.pauseAfterMs ?? 0 }
                : undefined;
        return {
            key: a.key,
