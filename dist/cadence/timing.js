 ...
 */
export function generateDwell(rng, profile) {
 ...
 */
export function generatePageReadDwell(pageKind, rng) {
    switch (pageKind) {
        case 'article':
 ...
 */
export function generatePostActionSettle(rng) {
 ...
 */
export function generateInterActionInterval(rng, profile) {
    const cadence = Number.isFinite(profile?.cadence) ? Math.max(0.25, Math.min(4, profile.cadence)) : 1;
