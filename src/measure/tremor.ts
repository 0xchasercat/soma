...
export function detrendTrajectory(path: TrajectoryPoint[]): {
  ...
  const windowSize = 7;
  if (path.length < windowSize) return { xResidual: [], yResidual: [] };
  const halfWin = Math.floor(windowSize / 2);
  const xResidual: number[] = [];
  const yResidual: number[] = [];
 ...
 */
export function hasMicroTremor(residual: number[]): boolean {
  if (residual.length < 12) return false;
  const meanSquare = residual.reduce((sum, value) => sum + value * value, 0) / residual.length;
  if (!Number.isFinite(meanSquare) || meanSquare < 1e-4) return false;
 ...
 */
export function computeAutocorrelation(signal: number[]): number {
  ...
  for (let i = 0; i < finite.length; i++) {
    ...
    if (i > 0) covariance += centered * (finite[i - 1]! - mean);
  }
  if (variance < 1e-12) return 0;
  return Math.max(-1, Math.min(1, covariance / variance));
}

function bandEnergy(signal: number[], sampleRateHz: number, lowHz: number, highHz: number): number {
 ...
 */
export function measureTremor(path: TrajectoryPoint[]): {
