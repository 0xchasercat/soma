...
export interface FlowSession {
  run(feeds: Record<string, FlowTensor>): Promise<Record<string, { data: Float32Array }>>;
}
...
export async function loadFlowModel(options: {
  ...
  const stats = JSON.parse(options.statsJson) as FlowStats;
...
function resampleToFrameRate(
  ...
  for (let i = 0; i < frames; i++) {
    ...
    while (src < t.length - 2 && t[src + 1]! < target) src++;
    const t0 = t[src]!;
    const t1 = t[src + 1]!;
    const span = t1 - t0;
...
export function reconstructGesture(
  ...
  for (let i = 0; i <= nSteps; i++) t[i] = (i / nSteps) * durationMs;
...
export async function synthesizeMovementFlow(
  ...
  const attempt = async (): Promise<{ points: TrajectoryPoint[]; durationMs: number }> => {
    ...
    for (let i = 0; i < dataDim; i++) z[i] = rng.nextGaussian(0, 1);
    ...
    const points: TrajectoryPoint[] = new Array(n);
    for (let i = 0; i < n; i++) {
 ...
 */
export async function synthesizeClickFlow(
  model: FlowModel,
  start: Point2D,
  target: TargetBox,
  profile?: Partial<BehaviorProfile>,
