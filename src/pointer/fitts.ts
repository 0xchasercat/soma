 ...
 */
export function sampleEndpoint(box: TargetBox, precision: number, rng: RNG): Point2D {
  ...
  if (precision >= 0.999 || width === 0 || height === 0) return { x: cx, y: cy };
 ...
 */
export function preMoveSettle(rng: RNG): number {
 ...
 */
export function clickHoldTime(rng: RNG): number {
