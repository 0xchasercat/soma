 ...
 */
export function classifyDigraph(prev: string, curr: string): DigraphClass {
  // Space transitions are a distinct class.
  if (prev === ' ') return 'after-space';

  // Punctuation transitions (non-alphanumeric, excluding space).
  if (/[^a-z0-9\s]/i.test(prev)) return 'after-punct';

  // Hand transition.
  const trans = handTransition(prev, curr);
  if (trans === 'same') return 'same-hand';
  if (trans === 'cross') return 'cross-hand';
 ...
 */
export const FLIGHT_TIME_PARAMS: Record<DigraphClass, { mu: number; sigma: number }> = {
 ...
 */
export const RELEASE_PRESS_PARAMS: Record<DigraphClass, {
  overlapProbability: number;
  overlapMagnitude: { mu: number; sigma: number };
  nonOverlap: { mu: number; sigma: number };
}> = {
  'same-hand': {
