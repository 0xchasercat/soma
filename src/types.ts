...
// ─── Synthesis Plans (pure data, no browser coupling) ────────────────────────
...
export type ModelFeatureVector = [
  number, // [0]  curvature (clamped, normalized)
  number, // [1]  submovement_count (clamped 0–5, / 5)
  number, // [2]  sigma_lognormal_fit R² [0,1]
  number, // [3]  jitter_autocorr normalized to [0,1]
  number, // [4]  has_micro_tremor (0 or 1)
  number, // [5]  velocity_profile == 'bell' (0 or 1)
  number, // [6]  path_length / straight_dist − 1, clamped [0,1]
  number, // [7] actual_duration / fitts_duration / 2, clamped [0,1]
  number, // [8] final_segment_speed / peak_speed, clamped [0,1]
  number, // [9]  max_overshoot / total_dist, clamped [0,0.3] / 0.3
  number, // [10] time_of_peak_speed / total_duration
  number, // [11] positive acceleration impulse / total acceleration impulse
];
...
export interface CapturedInteractionFeatures {
  ...
  interaction: CadenceFeatures & { count: number };
}
