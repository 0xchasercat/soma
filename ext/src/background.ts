...
const DB_NAME = 'soma_capture';
const DB_VERSION = 3;
const LEGACY_STORE_NAMES = ['movements', 'keystrokes', 'scrolls'] as const;
const STORE_NAMES = [...LEGACY_STORE_NAMES, 'interactions'] as const;
type StoreName = (typeof STORE_NAMES)[number];
type LegacyStoreName = (typeof LEGACY_STORE_NAMES)[number];
...
function isTrustedData(storeName: StoreName, data: unknown): boolean {
...
async function storeData(storeName: StoreName, data: unknown): Promise<void> {
...
async function getAllData(): Promise<{ movements: unknown[]; keystrokes: unknown[]; scrolls: unknown[]; interactions: unknown[] }> {
  const database = await openDB();
...
chrome.runtime.onMessage.addListener((message: unknown, _sender, sendResponse) => {
  ...
  if (type === 'store_movement') operation = storeData('movements', request.data).then(() => ({ ok: true }));
  else if (type === 'store_keystroke') operation = storeData('keystrokes', request.data).then(() => ({ ok: true }));
  else if (type === 'store_scroll') operation = storeData('scrolls', request.data).then(() => ({ ok: true }));
  else if (type === 'store_interaction') operation = storeData('interactions', request.data).then(() => ({ ok: true }));
  else if (type === 'get_all_data') operation = getAllData();
  else if (type === 'clear_all_data') operation = clearAllData().then(() => ({ ok: true }));
  else return false;
